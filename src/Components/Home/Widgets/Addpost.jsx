...
import { Avatar, Box, Grid, Button, TextField, Input, Typography, Autocomplete, InputLabel } from "@mui/material";
...
const Addpost = ({ onClose }) => {
  ...
  const fetchLocations = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/map/allfetchlocations`);
    ...
    const getUser = async () => {
      try {
        ...
        const [userResponse, profileResponse] = await Promise.all([
          axios.get(`${process.env.REACT_APP_API_URL}/api/profile`, { headers: { Authorization: token } }),
          axios.get(`${process.env.REACT_APP_API_URL}/api/profiledata`, { headers: { Authorization: token } }),
        ]);
  ...
  const handlePost = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/posts/newposts`, postData, {
        headers: {
          'Authorization': token,
  ...
  return (
    <Box sx={{ paddingLeft: { xs: 3 }, paddingRight: { xs: 3 }, maxHeight: '100%', overflow: 'hidden' }}>
      <WidgetWrapper style={{ display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 100px)', overflowY: 'auto' }}>
