  ...
  useEffect(() => {
    const fetchTravelGuideAndUserData = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/tg/fetchById/${guideId}`);
        ...
        if (travelGuideData) {
          const userResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/user/${travelGuideData.userId}`);
              ...
              <Box key={index} my={2} bgcolor="secondary.main" borderRadius={8} p={2} display="flex" flexDirection="column">
                ...
                {showImageId === destination.location_id && (
                  <div style={{ width: '250px', height: '250px', overflow: 'hidden', position: 'relative', borderRadius: '8px', marginTop: '8px' }}>
                    <img
                      src={`${process.env.REACT_APP_API_URL}/map/location/image/${destination.location_id}`}
